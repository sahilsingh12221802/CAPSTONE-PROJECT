...
class CattleBuffaloClassifier:
    ...
    def __init__(self, model_path):
        ...
        self.class_names = ['Gir', 'Holstein_Friesian', 'Jersey', 'Sahiwal', 'Jaffrabadi', 'Murrah', 'Other']
        self.gir_idx = 0
        self.sahiwal_idx = 3
    ...
    def predict_with_postprocessing(self, image_path_or_array, return_raw=False):
        ...
        species = breed_to_species.get(final_idx, 'Unknown') if final_idx >= 0 else 'Unknown'
        
        # All predictions sorted by confidence
        all_preds = [(self.class_names[i], float(predictions[i])) for i in range(len(self.class_names))]
    ...
    def _apply_postprocessing(self, predictions):
        ...
        if (gir_prob > self.gir_confidence_threshold and 
            gir_prob > sahiwal_prob / self.gir_vs_sahiwal_boost):
        ...
        if (sahiwal_prob > self.sahiwal_rejection_threshold and 
            gir_prob > 0.25 and 
        ...
        if max_prob > self.min_confidence:
            return max_idx, max_prob, (max_idx == self.gir_idx and max_prob <= self.gir_confidence_threshold)
        else:
            return -1, max_prob, True
    
    def set_postprocessing_params(self, gir_threshold=None, sahiwal_threshold=None, 
                                   gir_boost=None, min_conf=None):
...
if __name__ == '__main__':
    ...
    if test_images:
        ...
        for img_path in test_images:
            result = classifier.predict_with_postprocessing(str(img_path), return_raw=True)
            print(f"\n{img_path.name}:")
            ...
            if result['post_processed']:
                print(f"  Raw class: {result['raw_class']} ({result['raw_confidence']:.4f})")
            print(f"  Top-3: {', '.join([f'{c}:{p:.3f}' for c, p in result['all_predictions'][:3]])}")
