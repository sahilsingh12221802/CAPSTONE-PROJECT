...
gir_images = sorted(list(gir_test_dir.glob('*.jpg')) + list(gir_test_dir.glob('*.png')))[:15]
...
for img_path in gir_images:
    try:
        ...
        print(f"{status} {img_path.name:45} -> {str(breed):20} (species: {str(species):15} conf: {confidence:.4f})")
        if not is_correct:
            # Show top predictions for errors
            top = result.get('top_predictions', [])
            if top:
                top_str = ', '.join([(f"{p['label']}:{p['score']:.3f}") for p in top[:2]])
...
if gir_total > 0:
    ...
    print(f"  Correct (Gir):        {gir_correct}/{gir_total} = {100*gir_correct/gir_total:.1f}%")
    ...
    print(f"  Other misclassifications: {gir_total - gir_correct - gir_to_sahiwal - gir_to_unknown - gir_to_other}")
