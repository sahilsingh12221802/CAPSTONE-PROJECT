...
def post_process_predictions(predictions):
    ...
    if (
        gir_prob > GIR_CONFIDENCE_THRESHOLD
        and gir_prob > sahiwal_prob / GIR_VS_SAHIWAL_BOOST
    ):
    ...
    if (
        sahiwal_prob > SAHIWAL_REJECTION_THRESHOLD
        and gir_prob > 0.25
...
def diagnose_with_post_processing():
    ...
    for img_name in gir_images:
        ...
        post_idx, post_prob, post_note = post_process_predictions(predictions)

        # Get top 3 predictions for debugging
        top_3_idx = np.argsort(predictions[0])[-3:][::-1]
        top_3_str = ", ".join(
            [f"{CLASS_NAMES[i]}:{predictions[0, i]:.3f}" for i in top_3_idx]
        )
        ...
        if is_correct_standard:
            ...
            if raw_idx == SAHIWAL_IDX:
                sahiwal_failures.append(
                    (img_name, predictions[0, GIR_IDX], predictions[0, SAHIWAL_IDX])
                )
        ...
        print(
            f"{mark} {img_name:<50} -> "
            f"{CLASS_NAMES[post_idx] if post_idx >= 0 else 'None':<20} "
            f"(species: {species:<15} conf: {post_prob:.4f}) "
            f"{post_note:<25} {status}"
        )
        if post_idx != raw_idx:
    ...
    if sahiwal_failures:
        ...
        for name, gir_conf, sah_conf in sahiwal_failures:
            print(
                f"  {name}: Gir={gir_conf:.3f}, Sahiwal={sah_conf:.3f}, "
                f"diff={sah_conf - gir_conf:.3f}"
            )
